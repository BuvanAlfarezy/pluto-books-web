...
<body id="readerBody" class="bg-white text-black h-screen overflow-hidden transition">
    ...
    <main id="readerMain"
        ...
        <section id="readerPages"
            class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-24 md:h-full max-w-375 mx-auto font-serif text-[20px] md:text-[22px] leading-relaxed">
            <article id="leftPage" class="px-1 sm:px-6 md:px-8 overflow-hidden"></article>
    ...
    <footer id="readerFooter"
        ...
        <div class="max-w-375 mx-auto relative flex items-center justify-center">

            <div id="readerPagination"
                class="hidden md:flex items-center gap-4 border px-5 md:px-8 py-3 rounded-full bg-white text-black">
                <button onclick="prevPage()"
                    class="w-10 h-10 flex items-center justify-center rounded-full text-2xl hover:bg-gray-100 transition">
                    ‹
                </button>

                <span id="pageInfo" class="text-sm md:text-base whitespace-nowrap min-w-27.5 text-center">
                    Page 1 of 1
                </span>
            ...
            <button id="finishButton" onclick="finishReading()" disabled
    ...
    <!-- Custom Modal -->
    <div id="readerModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 px-5">
        <div id="readerModalBox" class="w-full max-w-sm rounded-3xl bg-white text-black p-7 shadow-xl border">
            ...
            <button id="readerModalButton" onclick="closeModal()"
    ...
    function openModal(title, message) {
        ...
        if (isDark) {
            modalBox.className =
                'w-full max-w-sm rounded-3xl bg-[#1c1c1c] text-white p-7 shadow-xl border border-gray-700';
    ...
    ` :
            `
      <div class="flex flex-col justify-center h-full text-center px-4">
  <p class="text-lg mb-6 tracking-wide">CHAPTER</p>

  <h1 class="text-3xl md:text-4xl leading-snug border-b pb-6 break-words">
    ${bookTitle}
  </h1>
</div>
    `;
    ...
    function applyTheme() {
        if (isDark) {
            ...
            header.className =
                'fixed top-0 left-0 right-0 z-30 bg-[#111]/95 backdrop-blur px-3 py-3 md:px-8 md:py-5 flex flex-col md:flex-row md:justify-between md:items-center gap-3';

            toolbar.className =
                'w-full md:w-auto flex items-center justify-between md:justify-center gap-2 md:gap-6 border border-gray-700 rounded-full px-4 md:px-7 py-3 bg-[#1c1c1c] text-white shadow-sm text-sm md:text-base';

            footer.className =
                'fixed bottom-0 left-0 right-0 z-30 bg-[#111] text-white border-t border-gray-700 px-4 md:px-8 py-3 md:py-4';

            pagination.className =
                'hidden md:flex items-center gap-4 border border-gray-700 px-5 md:px-8 py-3 rounded-full bg-[#1c1c1c] text-white';
        } else {
            body.className = 'bg-white text-black h-screen overflow-hidden transition';

            header.className =
                'fixed top-0 left-0 right-0 z-30 bg-white/95 backdrop-blur px-3 py-3 md:px-8 md:py-5 flex flex-col md:flex-row md:justify-between md:items-center gap-3';

            toolbar.className =
                'w-full md:w-auto flex items-center justify-between md:justify-center gap-2 md:gap-6 border rounded-full px-4 md:px-7 py-3 bg-white text-black shadow-sm text-sm md:text-base';

            footer.className =
                'fixed bottom-0 left-0 right-0 z-30 bg-white text-black border-t px-4 md:px-8 py-3 md:py-4';

            pagination.className =
                'hidden md:flex items-center gap-4 border px-5 md:px-8 py-3 rounded-full bg-white text-black';
        }
